...
In Exercise 01, we showed that the log-liklihood can be written as

\[
  \nabla \ell(\beta) = \sum\limits_i (y_i - m_iw_i(\beta)) x_i
\]

The first term in the parenthesis is the actual value of the observation, the
second term is the expected value given the current guess for the parameters $\beta$

\section{Unbiased Estimators}

We want to show that $n g_i(\beta)$ is an unbiased estimator of the gradient:

\begin{align*}
  \E\{n g_i(\beta)\}
  &= n \E\{g_i(\beta)\} \\
  &= n \left( \frac{1}{n} \sum\limits_{i=1}^n g_i(\beta) \right)\\
  &= \sum\limits_{i=1}^n g_i(\beta)\\
  &= \nabla \ell(\beta)
\end{align*}
